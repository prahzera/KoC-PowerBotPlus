...
Tabs.Research = {
	...
	init: function (div) {
		...
		m += '<td colspan=2 align=right>' + tx('Current Research Cost') + ':&nbsp;<span id=btResearchCostTR></span>&nbsp;&nbsp;</td></tr>';
		...
		for (var h in t.researchinfo) {
			var rowClass = (++r % 2) ? 'evenRow' : 'oddRow';
	...
	createToolTip: function (elem) {
		...
		var slot, briton, name, description, nxt, max, food, wood, stone, ore, gold, buildings, research;
		if (typ == "Rsc") {
			slot = 1;
			briton = false;
			nxt = parseIntNan(Seed.tech["tch" + h]) + 1;
			max = uW.Research.Methods.maxLevel(h, 1);
			name = t.researchinfo[h].name;
			description = t.researchinfo[h].description;
			food = parseInt(uW.techcost["tch" + h][1] * nxt * cost);
			wood = parseInt(uW.techcost["tch" + h][2] * nxt * cost);
			stone = parseInt(uW.techcost["tch" + h][3] * nxt * cost);
			ore = parseInt(uW.techcost["tch" + h][4] * nxt * cost);
			gold = parseInt(uW.techcost["tch" + h][5] * nxt * cost);
			buildings = uW.techcost["tch" + h][8];
			research = uW.techcost["tch" + h][9];
		}
		else {
			slot = 2;
			briton = true;
			name = t.britoninfo[h].name;
			description = t.britoninfo[h].description;
			nxt = parseIntNan(Seed.tech2["tch" + h]) + 1;
			max = uW.Research.Methods.maxLevel(h, 2);
			food = parseInt(uW.techcost2["tch" + h][1] * nxt * cost);
			wood = parseInt(uW.techcost2["tch" + h][2] * nxt * cost);
			stone = parseInt(uW.techcost2["tch" + h][3] * nxt * cost);
			ore = parseInt(uW.techcost2["tch" + h][4] * nxt * cost);
			gold = parseInt(uW.techcost2["tch" + h][5] * nxt * cost);
			buildings = uW.techcost2["tch" + h][8];
			research = uW.techcost2["tch" + h][9];
		}
		...
		if (research.length == null) {
			for (var i = 0; i < k.length; i++) {
				...
				if (parseInt(research[k[i]][0]) == 1) {
					req = parseInt(research[k[i]][1]);
				} else {
					req = nxt + parseInt(research[k[i]][1]);
				}
				var span = '<span>';
		...
		if (buildings.length == null) {
			for (var i = 0; i < k.length; i++) {
				...
				if (parseInt(buildings[k[i]][0]) == 1) {
					req = parseInt(buildings[k[i]][1]);
				} else {
					req = Math.min(uW.buildingmaxlvl[p], nxt + parseInt(buildings[k[i]][1]));
				}
				var v = [9, 15, 16, 17],
					K = [5, 9, 12, 14];
	...
	PaintOverview: function () {
		...
		for (var i = 0; i < Cities.numCities; i++) {
			...
			var blvl = [];
			for (var bpos in Seed.buildings["city" + cityId]) {
	...
	getResearchTime: function (cityId, tech, type) {
		...
		var base;
		if (type == 1) {
			D = parseIntNan(Seed.tech["tch" + tech]) + 1;
			base = parseInt(uW.techcost["tch" + tech][7]);
		}
		if (type == 2) {
			D = parseIntNan(Seed.tech2["tch" + tech]) + 1;
			base = parseInt(uW.techcost2["tch" + tech][7]);
		}
	...
	checkResearching: function (id, briton) {
		...
		var t = Tabs.Research;
		var qres;
		if (briton) { qres = Seed.queue_tch2; }
		else { qres = Seed.queue_tch; }
	...
	doAutoLoop: function (idx, briton) {
		...
		var now = unixTime();
		var qres, slot;
		if (briton) { qres = Seed.queue_tch2["city" + cityId]; slot = 2; }
		else { qres = Seed.queue_tch["city" + cityId]; slot = 1; }
		if (qres == null) qres = [];
		var len = qres.length;
		if ((len == 0 || (qres[len - 1][3] - now) < 0)) {
			if (!Options.ResearchOptions.ThroneCheck || (t.researchspeed >= Options.ResearchOptions.ResearchSpeed)) {
				...
				if (Options.ResearchOptions.Enabled[idx] && ascensionok) {
					if (t.citydelay[idx] > 0) { t.citydelay[idx]--; }
					else {
		...
		if (!briton) {
			...
			if (Seed.cityData.city[cityId].isPrestigeCity && parseIntNan(Seed.cityData.city[cityId].prestigeInfo.prestigeType) == 3) {
		...
		if (idx == Cities.numCities) {
			if (!t.loopaction) { t.autodelay = t.intervalSecs; }
	...
	eventSelectResearch: function (cityId, idx, slot) {
		...
		var h;
		if (slot == 1) {
			for (h in Options.ResearchOptions.ResearchNumbers) {
			...
			for (h in Options.ResearchOptions.BritonNumbers) {
				if (t.checkRequirements(h, cityId, idx, slot)) {
		...
		if (t.tableau.length == 0) return;

		if (parseIntNan(Options.ResearchOptions.ResearchPriority) == 1) {
			t.tableau.sort(function (a, b) { return a.time - b.time; });
		}
		if (parseIntNan(Options.ResearchOptions.ResearchPriority) == 2) {
			t.tableau.sort(function (a, b) { return a.combat - b.combat; });
		}
	...
	checkRequirements: function (h, cityId, idx, slot) {
		...
		if (slot == 1) {
			nxt = parseIntNan(Seed.tech["tch" + h]) + 1;
			res = uW.techcost["tch" + h];
		}
		if (slot == 2) {
			nxt = parseIntNan(Seed.tech2["tch" + h]) + 1;
			res = uW.techcost2["tch" + h];
		}
		...
		new MyAjaxRequest(uW.g_ajaxpath + ajaxaddress + uW.g_ajaxsuffix, {
			...
			onSuccess: function (rslt) {
				if (rslt.ok) {
					...
					var utstart, uteta, queue;
					if (slotNum == 1) {
						utstart = parseInt(Seed.queue_tch["city" + cityId][0][2]);
						uteta = parseInt(Seed.queue_tch["city" + cityId][0][3]);
						queue = Seed.queue_tch;
					}
					if (slotNum == 2) {
						utstart = parseInt(Seed.queue_tch2["city" + cityId][0][2]);
						uteta = parseInt(Seed.queue_tch2["city" + cityId][0][3]);
						queue = Seed.queue_tch2;
					}

					var timered = SpeedupArray[parseInt(item) - 1];
	...
	Research: function (cityId, idx, slotNum, h) {
		...
		var chk = uW.checkreq("tch", h, lvl, (slotNum == 2) ? slotNum : undefined);
		uW.currentcityid = saveCityId;
		for (var c = 0; c < chk[3].length; c++) {
			if (chk[3][c] == 0) {
				t.citydelay[citynum] = 10;
				if (slotNum == 2) {
					actionLog(Cities.byID[cityId].name + ': Final research check failed (' + uW.techcost2['tch' + h][0] + ' Level ' + lvl + ')', 'RESEARCH');
				}
				else {
					actionLog(Cities.byID[cityId].name + ': Final research check failed (' + uW.techcost['tch' + h][0] + ' Level ' + lvl + ')', 'RESEARCH');
				}
				return;
			}
		}
		...
		new MyAjaxRequest(uW.g_ajaxpath + ajaxaddress + uW.g_ajaxsuffix, {
			...
			onSuccess: function (rslt) {
				if (rslt.ok) {
					...
					if (rslt.error_code == 111) {
						if (slotNum == 2) {
							uW.techcost2["tch666"] = uWCloneInto(["Researching...", 0, 0, 0, 0, 0, 0, 0, [], [], ""]);
						}
						else {
							uW.techcost["tch666"] = uWCloneInto(["Researching...", 0, 0, 0, 0, 0, 0, 0, [], [], ""]);
						}
